/**
 * 매물 등록 화면
 * TODO-DB: 저장 시 Supabase `properties` insert
...
export default function PropertyRegisterScreen({ embedded = false }: ScreenProps) {
  ...
  const [deal, setDeal] = useState<DealKind>('월세'); // 거래종류(기본 월세)
  const [propType, setPropType] = useState<PropKind>('아파트'); // 매물종류
  const [salePrice, setSalePrice] = useState<string>(''); // 매매가(거래 매매)
  const [jeonsePrice, setJeonsePrice] = useState<string>(''); // 전세금(거래 전세)
  const [deposit, setDeposit] = useState<string>(''); // 보증금(거래 월세)
  const [monthly, setMonthly] = useState<string>(''); // 월세(거래 월세)
  const [areaSqm, setAreaSqm] = useState<string>(''); // 면적 ㎡
  const [floor, setFloor] = useState<string>(''); // 해당층
  const [totalFloors, setTotalFloors] = useState<string>(''); // 총층
  const [direction, setDirection] = useState<string>(''); // 방향
  const [moveInDate, setMoveInDate] = useState<string>(''); // 입주일
  const [ownerName, setOwnerName] = useState<string>(''); // 집주인 이름
  const [relation, setRelation] = useState<RelationKind | undefined>(undefined); // 관계(미선택 시 undefined)
  const [ownerPhone, setOwnerPhone] = useState<string>(''); // 전화(하이픈 표시)
  const [ownerMemo, setOwnerMemo] = useState<string>(''); // 집주인 섹션 메모
  const [memo, setMemo] = useState<string>(''); // 메모

  const [showSuggest, setShowSuggest] = useState<boolean>(false); // 자동완성 표시
  ...
  /** 저장 (DB 미연동) */
  const onSave = () => {
    // TODO-DB: supabase.from('properties').insert({ moveInDate: moveInDate || undefined, ... })
  };
